...
import { getLastUserText } from "./message-utils.js";
import {
  createCoachWarning,
  createEmptyCoachTurnData,
  normalizeCoachTurnData,
} from "./types.js";
...
export function extractMentionedEquipment(message) {
  const normalized = normalizeMessage(message);
...
export function mergeEquipment(existingItems = [], incomingItems = []) {
...
function toSuggestedActions(reply) {
  return [
...
function buildCoachReply(content, context = {}) {
  ...
  const activityCount = context.upcomingActivities?.length ?? 0;
...
function followUpItemToOperations(followUp) {
  const operations = [];

  if (followUp?.habit) {
    operations.push({
      type: "create_habit",
      habit: followUp.habit,
      ...(followUp.activity ? { activity: followUp.activity } : {}),
    });
    return operations;
  }
...
function operationKey(operation) {
  switch (operation.type) {
    case "create_habit":
      return `${operation.type}:${operation.habit.id}`;
    case "create_task":
      return `${operation.type}:${operation.task.id}`;
    case "create_activity":
      return `${operation.type}:${operation.activity.id}`;
    case "add_equipment":
      return `${operation.type}:${operation.equipmentItems.map((item) => item.id).sort().join(",")}`;
    case "suggest_action":
      return `${operation.type}:${operation.suggestion.message}`;
    default:
...
export function dedupeOperations(operations = []) {
  ...
  operations.forEach((operation) => {
    if (operation.type === "add_equipment") {
      operation.equipmentItems.forEach((item) => {
  ...
  if (equipmentById.size > 0) {
    unique.push({
      type: "add_equipment",
      equipmentItems: Array.from(equipmentById.values()),
    });
  }
...
export function buildMockCoachTurn({
  userMessage,
  teamId,
  conversationId,
  context = {},
  ...
  const mentionedEquipment = extractMentionedEquipment(userMessage);
  const existingEquipmentIds = new Set((context.equipmentItems ?? []).map((item) => item.id));
  ...
  const operations = dedupeOperations([
    ...followUpItemToOperations(followUp),
    ...(newlyMentionedEquipment.length > 0
      ? [
          {
            type: "add_equipment",
            equipmentItems: newlyMentionedEquipment,
          },
        ]
      : []),
  ]);
  ...
  return {
    text: reply.content,
    coachTurn: normalizeCoachTurnData({
      suggestedActions: toSuggestedActions(reply),
      operations,
      warnings,
    }),
  };
...
export function createMockWarningForLiveFallback(error) {
  if (!error) {
    return null;
  }

  return createCoachWarning(
    "live_fallback",
...
export function buildMockTurnFromMessages({
  messages,
  conversationId,
  teamId,
  context,
...
export function emptyMockTurn() {
  return {
    text: "",
    coachTurn: createEmptyCoachTurnData(),
  };
}
